reference = "Reference: children: 3, cats: 7, samoyeds: 2, pomeranians: 3, akitas: 0, vizslas: 0, goldfish: 5, trees: 3, cars: 2, perfumes: 1"
input = "Sue 1: children: 1, cars: 8, vizslas: 7
...
defmodule Parser do
  ...
  @matchers [@children, @cats, @samoyeds, @pomeranians, @akitas, @vizslas, @goldfish, @trees, @cars, @perfumes]

  def parse(string) do
    @matchers
    |> Enum.map(&match_category(&1, string))
  end

  def match_category(matcher, string) do
    case Regex.run(matcher, string) do
      [_string, number] -> String.to_integer(number)
      _ -> nil
    end
  end
end

defmodule SueFinder do
  @fns [:==, :<, :==, :>, :==, :==, :>, :<, :==, :==]

  def find_index(sues, string) do
    reference = Parser.parse(string)
    Enum.find_index(sues, &compare(reference, &1)) + 1
  end

  def compare(reference, sue) do
    [reference, @fns, sue]
    |> List.zip()
    |> Enum.all?(fn {r, f, s} -> is_nil(s) || apply(Kernel, f, [r, s]) end)
  end
end
...
|> SueFinder.find_index(reference)
|> inspect()
|> IO.puts()