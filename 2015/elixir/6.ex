input = "turn on 489,959 through 759,964
...
defmodule Board do
  ...
  def execute(board, command) do
    {action, {x1, y1}, {x2, y2}} = Command.parse(command)
    coords = for x <- x1..x2, y <- y1..y2, into: [], do: {x, y}
    Enum.reduce(coords, board, fn c, b -> apply(Board, action, [b, c]) end)
  end

  def off(board, coord) do
    Map.update!(board, coord, fn val -> max(0, val - 1) end)
  end
  def on(board, coord) do
    Map.update!(board, coord, &(&1 + 1))
  end
  def toggle(board, coord) do
    Map.update!(board, coord, &(&1 + 2))
  end
end
...
defmodule Command do
  def parse(string) do
    ...
    [first, second] = extract_coords(string)
    {action, first, second}
  end

  def extract_coords(string) do
    [x1, y1, x2, y2] =
      ~r/(\d+),(\d+) through (\d+),(\d+)/
      |> Regex.run(string)
      |> Enum.drop(1)
  ...
  def action("turn on " <> _rest), do: :on
  def action("turn off " <> _rest), do: :off
  def action("toggle " <> _rest), do: :toggle
end
...
input
|> String.split("\n")
|> Enum.reduce(board, fn c, b -> Board.execute(b, c) end)
|> Map.values()
|> Enum.sum()
|> IO.puts()