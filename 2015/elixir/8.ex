...
defmodule Rater do
  def rate(string) do
    {code_length(string), memory_length(string)}
  end

  def code_length(string) do
    string
    |> String.codepoints()
    |> length()
  end

  def memory_length(string) when is_binary(string) do
    string
    |> String.codepoints()
    |> Enum.slice(1..-2)
    |> memory_length()
  end
  def memory_length(sequence), do: memory_length(sequence, 0)
  def memory_length(["\\", "x", _one, _two | rest], count), do: memory_length(rest, count + 1)
  def memory_length(["\\", _one | rest], count), do: memory_length(rest, count + 1)
  def memory_length([_ | rest], count), do: memory_length(rest, count + 1)
  def memory_length([], count), do: count
end
...
defmodule Encoder do
  ...
  def encode(["\"" | rest], result), do: encode(rest, ["\"", "\\" | result])
  def encode(["\\" | rest], result), do: encode(rest, ["\\", "\\" | result])
  def encode([point | rest], result), do: encode(rest, [point | result])
  def encode([], result), do: ["\"" | result] |> Enum.reverse |> List.to_string()
end

input
|> String.split("\n")
|> Enum.map(&Rater.rate/1)
|> Enum.map(fn {code, memory} -> code - memory end)
|> Enum.sum()
|> IO.puts()
...
|> String.split("\n")
|> Enum.map(fn str ->
  {original, _} = Rater.rate(str)
  {new, _} = str |> Encoder.encode() |> Rater.rate()
  new - original
end)
|> Enum.sum()
|> IO.puts()