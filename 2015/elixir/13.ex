...
defmodule Parser do
  @matcher ~r/^(\w+) would (gain|lose) (\d+) [\w ]+ to (\w+)./

  def parse(string, acc) do
    [_, guest, direction, magnitude, neighbor] = Regex.run(@matcher, string)

    happy_value =
      direction
      |> case do
        "gain" -> String.to_integer(magnitude)
        "lose" -> String.to_integer(magnitude) * -1
      end

    Map.update(acc, guest, %{neighbor => happy_value}, &Map.put(&1, neighbor, happy_value))
  end
end

defmodule Table do
  def permute([a]), do: [ [a] ]
  def permute(guest_list) do
    for guest <- guest_list, remainder <- permute(guest_list -- [guest]), into: [], do: [guest | remainder]
  end

  def calculate_happiness([h | tail], index), do: calculate_happiness([h] ++ tail ++ [h], index, 0)
  def calculate_happiness([_one], _index, acc), do: acc
  def calculate_happiness([one, two | tail], index, acc) do
    pair_value = get_happiness(index, [one, two]) + get_happiness(index, [two, one])
    calculate_happiness([two | tail], index, acc + pair_value)
  end

  def get_happiness(index, path), do: get_in(index, path) || 0
end

seating_values =
  input
  |> String.split("\n")
  |> Enum.reduce(%{}, &Parser.parse/2)

seating_values
|> Map.keys()
|> Kernel.++(["Me"])
|> Table.permute()
|> Enum.map(&Table.calculate_happiness(&1, seating_values))
|> Enum.max()
|> inspect
|> IO.puts()